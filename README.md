...
| `02_pandas/` | Series fundamentals · load & inspect · clean/missing/dtypes · select/filter/transform · groupby/pivot · merge/join/concat · time series · pitfalls & debugging | the bulk of the interview: reading, cleaning, reshaping, joining, time handling, and the catalogue of things that go wrong |
| `03_scipy/` | stats · optimize/interpolate/signal | hypothesis tests, distributions, curve fitting, small constrained optimisation |
| `04_sklearn/` | regression workflow · preprocessing & pipelines · cross-validation for time series · diagnostics & interpretation | boring-but-correct modelling, honest evaluation, critiquing a result |
| `05_time_series_research/` | features/targets/leakage · evaluation & baselines · end-to-end template | the Fuse-style flow: define the problem, build honest features, beat the naive baseline, explain the errors |
...
| `06_mocks/` | 13 deliberately broken research notebooks + `solutions/` | the interview simulation: open one blind, 45 minutes, find the planted bugs, leakage and questionable choices, then check the answer key |
...
5. `01_numpy/*`, `02_pandas/03`–`05`, `03_scipy/*`, `04_sklearn/02`, `07_optimisation/03`, `04`, `06` as reference.
