...
| `03_scipy/` | **stats intro from scratch** (SE, t-stats, autocorrelation, regression CIs, with the maths) · stats · optimize/interpolate/signal · sampling distributions, standard errors & bootstrap · hypothesis tests, power & pitfalls · **t-statistics from scratch** (theory, synthetic, real data) | hypothesis tests, distributions, curve fitting, small constrained optimisation |
...
6. `03_scipy/00` (statistics from scratch, read this first if the maths feels hazy), then `03_scipy/05` (t-statistics end to end) and `03_scipy/03`–`04` (bootstrap, hypothesis tests, power) and `05_time_series_research/07` (statistical traps) — for the "is this result real?" questions.
